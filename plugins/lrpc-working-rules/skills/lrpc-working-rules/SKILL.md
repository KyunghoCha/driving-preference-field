---
name: lrpc-working-rules
description: Use when a task changes code, docs, formulas, tests, or experiment workflow inside the local-reference-path-cost repo. First study the relevant external references when network access is available, then read the repo operating docs and define the clean baseline, hypothesis, and verification boundary before making changes.
---

# LRPC Working Rules

Use this skill for work inside the `local-reference-path-cost` repository.
