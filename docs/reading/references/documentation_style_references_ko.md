...
> 참고용 reference log다. 현재 문서 작성 판단은 `docs/explanation/documentation_writing_principles_ko.md`를 기준으로 읽는다.
...
- 우리 repo 적용 포인트:
  ...
  - `docs/index.md`가 문서 포털을 맡고, 개별 문서는 자기 역할의 질문에만 답하게 한다
...
- 출처: `https://kubernetes.io/docs/contribute/style/style-guide/`
- 참고한 핵심 원칙:
...
- 우리 repo 적용 포인트:
  ...
  - `docs/index.md`에서 newcomer spine을 고정하고, 개별 문서 H1은 번호 없이 유지한다
  - 문서/코드/테스트가 공유하는 공개 용어는 표기와 의미를 같이 맞춘다

## 5. Red Hat Modular Documentation

- 출처: `https://redhat-documentation.github.io/modular-docs/`
- 참고한 핵심 원칙:
...
- 우리 repo 적용 포인트:
  ...
  - `parameter_lab_ko.md`는 prerequisite, steps, expected outputs, limits 순서의 task-first 문서로 유지한다

## 현재 기준

이 문서는 공식 출처와 해석 포인트만 기록한다.
실제 판단 규칙은 `docs/explanation/documentation_writing_principles_ko.md`에 두고,
