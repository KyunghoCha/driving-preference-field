...
`docs/raw/` is the owner-origin capture layer for Korean raw notes, change tracking, and the latest owner design notebook. It is intentionally non-canonical. Use it to preserve source material and idea history, not to replace explanation, reference, how-to, or status pages.
