...
현재 GUI는 canonical 전체를 다 노출하지는 않는다. progression frame, longitudinal, transverse, support ceiling 축을 직접 다루는 compare tool에 집중돼 있고, interior_boundary, continuity_branch, exception layer 파라미터는 아직 직접 노출하지 않는다.
