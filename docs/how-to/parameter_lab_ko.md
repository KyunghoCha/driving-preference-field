...
상단 툴바에는 빠른 액션만 둔다. `Reload Case`, `Export Comparison`, `Reset View`와 `channel`, `scale` selector가 있고, `Lab Help`로 이 문서를 앱 안에서 바로 열 수 있다. 기본 shortcut은 `F5`(reload), `Ctrl+Shift+E`(export), `Ctrl+0`(reset view), `F1`(lab help)다.
