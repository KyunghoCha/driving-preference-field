...
상단 툴바에는 빠른 액션만 둔다. `Reload Case`, `Export Comparison`, `Reset View`와 `channel`, `scale` selector가 있고, `Docs`로 이 문서를 앱 안에서 바로 열 수 있다. 기본 shortcut은 `F5`(reload), `Ctrl+Shift+E`(export), `Ctrl+0`(reset view), `F1`(docs)다.

`Docs`와 `Parameter Help`는 역할이 다르다. `Docs`는 Parameter Lab 전체 사용 흐름과 현재 도구 구조를 설명하는 문서 브라우저이고, 우측 `Parameters` 패널의 `Help` 버튼은 현재 파라미터 의미와 current implementation truth를 요약한 parameter guide다.
