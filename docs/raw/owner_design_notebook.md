...
핵심은 입력 경로가 주어졌다고 해서 그 선을 그대로 복제하는 것이 아니라, 그 경로를 바탕으로 progression과 branch ordering을 읽는 기준을 제공하는 데 있다. DPF는 그 기준을 이용해 whole-space preference를 만든다.
...
같은 이유로 DPF가 local corridor를 지나치게 강하게 붙잡는 장치가 되는 것도 피하는 편이 맞다. 경로를 완전히 잃지 않게 방향을 주는 것과, centerline에 계속 고정시키는 것은 다른 일이다. 후자는 planner와 behavior 쪽 책임이 더 크다.
...
입력 경로가 있기 때문에 DPF는 현재 진행이 어느 구조와 연결돼 있는지, 어느 쪽이 더 앞선 진전인지, reverse 같은 경우에도 어떤 방향을 progression으로 볼지를 정의할 수 있다. 그러나 그 입력 경로가 곧 실제 주행 경로라는 뜻은 아니다.
...
이 경계가 분명해야 DPF가 centerline-following device로 과도하게 비대해지지 않는다. 반대로 DPF가 너무 약하면 planner가 참고할 preference spine 자체가 사라진다. 따라서 역할을 없애는 것이 아니라, preference와 execution을 구분하는 것이 핵심이다.
