...
이 문서는 기반 선호장(`base driving preference field`)이 무엇인지, 왜 필요한지, 그리고 현재 구현이 그 개념을 어떻게 근사하는지 설명한다. 여기서 고정하려는 것은 exact formula가 아니라 개념 모델이다. 즉 어떤 수식을 쓰더라도 이 field가 무엇을 표현해야 하는지는 이 문서에서 분명해야 한다.
...
이 field는 winner direction selector가 아니다. 공간 안에서 어디가 더 높은 preference를 가지는지, 어떤 continuation이 더 자연스러운지, 어떤 상태와 trajectory가 intended progression과 더 잘 맞는지를 설명하는 구조다. winner 결정과 최종 trajectory 형성은 상위 layer나 optimizer가 맡는다.
...
둘은 모두 필요하지만 서로를 대신하지는 않는다. drivable만으로는 무엇이 앞이고 뒤인지가 흐려질 수 있고, progression만으로는 지금 당장 어디를 움직일 수 있는지까지 보장하지 못한다. 그래서 base field는 둘을 한 덩어리로 섞기보다, 서로 다른 의미를 가진 입력으로 읽고 같은 surface 위에서 함께 반영해야 한다.
...
따라서 support와 gate는 ordering을 약하게 조절할 수는 있어도, field의 주축을 결정해서는 안 된다. field는 여전히 공간의 ordering을 알려주고, winner 결정은 상위 layer가 맡는다는 분리가 유지돼야 한다.

## 현재 구현은 이 개념을 어떻게 근사하는가

현재 구현은 nearest winner를 고르는 방식 대신 smooth skeleton anchor를 좌표 control point로 보고, Gaussian elliptical blend로 whole-fabric continuous function을 만든다. branch 사이도 guide별 patch를 따로 만들기보다 같은 함수 안에서 fabric-like surface로 메운다.
...
구조적으로 보면 현재 구현은 `transverse_component + longitudinal_component` 위에 `support_mod`와 `alignment_mod`가 약한 secondary modulation으로 얹히는 형태다. strong longitudinal 설정에서는 farther-ahead off-center point가 nearer-center point보다 높아질 수도 있다. exact formula는 별도 수식 문서에 정리하지만, 그 수식도 결국 이 개념 모델을 만족하는지로 읽어야 한다.
