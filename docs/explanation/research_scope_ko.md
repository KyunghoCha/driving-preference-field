...
또 이 연구는 외부 semantics를 받아 field를 생성하는 구조를 다룬다. 이 단계에서 주행 가능 공간을 perception이나 HD map processing으로 직접 추출하는 문제를 풀지는 않는다. drivable semantics와 progression semantics는 외부에서 주어진다고 가정하고, 이 입력이 어떻게 field의 base layer를 만들게 하는지를 다룬다.
