...
2. exception layer가 soft / hard burden을 어떻게 만든는가
...
detail channel은 dominant guide 기준 값을 기본으로 내보낸다. 다만 exported `progression_transverse_component`만 handoff candidate 집합 안에서 부드럽게 섞는다.

candidate 집합:

```math
\mathcal{C}(p)=\left\{g\ \middle|\ \text{support\_sum}_g \ge 0.25\,\text{support\_sum}_{dom},\ \text{score}_g \ge \text{score}_{dom}-0.20 \right\}
```

weight:

```math
w_g \propto \text{support\_sum}_g \exp\left(\frac{\text{score}_g-\text{score}_{dom}}{0.05}\right)
```

smoothed transverse export:

```math
\text{transverse\_component}(p)=\sum_{g\in\mathcal{C}(p)} \tilde{w}_g\, T(r_g)
```
