...
progression surface는 local map 전체에 진행 좌표와 횡방향 좌표를 동시에 부여하기 위해 필요하다. 현재 구현은 smooth skeleton anchor를 control point로 쓰되, guide-local 좌표와 score를 먼저 만든 뒤 guide 간 pointwise max envelope를 취한다. visible guide endpoint는 virtual continuation으로 처리하고, support/alignment는 weak secondary modulation으로 남긴다.
