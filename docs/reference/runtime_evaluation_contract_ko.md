...
- evaluator config
- QueryContext (`ego_pose`, `local_window`, optional mode/phase)
- optional mode / phase context

Phase 5 current truth에서는 raw source가 바로 runtime으로 들어오지 않는다.

- raw source -> source adapter
- source adapter -> `SemanticInputSnapshot + QueryContext`
- runtime -> canonical output contract 소비

즉 runtime은 toy loader output과 generic source adapter output을 같은 canonical contract로 다룬다.
...
Phase 5 current public runtime interface는 다음으로 기록한다.
...
Phase 5 current runtime contract는 다음을 만족해야 한다.
