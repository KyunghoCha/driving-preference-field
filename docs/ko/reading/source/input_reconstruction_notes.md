...
현재 `progression_surface.py`는 Gaussian anchor blend로 guide-local coordinate를 만들고 hard max envelope를 사용한다. exported transverse도 dominant guide의 실제 transverse term을 그대로 쓴다. projection-based coordinate 실험은 롤백했다.
...
- `split_branch` / `merge_like_patch`:
  - hard max envelope가 만드는 morphology가 현재 semantics와는 일관되지만, split/merge 내부와 바깥쪽 경계를 얼마나 더 자연스럽게 만들지는 추가 검토가 필요하다.

즉 다음 품질 배치에서 볼 핵심은 “guide-local coordinate를 다시 바꿀 것인가”보다, “현재 blended coordinate와 dominant-guide transverse를 어디까지 더 단순하고 일관되게 유지할 것인가”다.
