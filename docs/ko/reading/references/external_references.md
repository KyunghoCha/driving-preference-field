...
| Gaussian / normalized anchor blending | Donald Shepard, *A two-dimensional interpolation function for irregularly-spaced data*, Proceedings of the 1968 23rd ACM National Conference, 517-524, 1968. DOI: `10.1145/800186.810616` | 현재 구현에서 anchor를 winner로 고르지 않고 **정규화된 부드러운 pooled weight blend**로 coordinate와 support를 함께 추정하는 방향을 정리할 때 참고했다. | `src/driving_preference_field/progression_surface.py`, `docs/ko/reference/runtime_evaluation_contract.md`, `docs/ko/how-to/parameter_lab.md` |
...
- 현재 구현은 Gaussian anchor blended pooled coordinates + soft progress gating 방식이다.
