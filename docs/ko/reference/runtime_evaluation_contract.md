...
현재 구현은 progression anchor 전체를 한 pooled blended coordinate field로 읽고, provisional pooled progress 주위에 soft progress gate를 한 번 더 곱한 뒤 final score를 한 번만 계산한다. guide 간 hard max envelope는 더 이상 없다. 대신 exported `progression_transverse_component`는 split/merge inspection 품질을 위해 eligible guide들 사이의 guide-aware smoothing을 유지한다. `safety_soft`, `rule_soft`, `dynamic_soft`, hard mask는 visualization / costmap 성격의 burden channel로만 남고 public runtime payload에는 싣지 않는다.
