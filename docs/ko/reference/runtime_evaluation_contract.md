...
현재 구현은 progression anchor 전체를 하나의 pooled blended coordinate field로 평가한다. 먼저 provisional pooled progress를 계산하고 soft progress gating을 적용한 뒤, 같은 pooled field에서 `s_hat`, `n_hat`, tangent, longitudinal component, transverse component를 읽는다. exported `transverse_component`는 inspection 근사가 아니라 `progression_tilted`에 실제로 들어가는 pooled transverse term과 동일하다.
