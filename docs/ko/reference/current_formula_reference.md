...
각 progression guide는 자기 anchor만으로 local tangent / normal 좌표를 만든다. 이 좌표가 뒤에서 guide-local `s_hat`와 local tangent를 만드는 기본 재료가 된다.
...
이 단계에서 guide-local `s_hat`, `t_hat`를 만든다. `center_distance`는 같은 dominant guide의 raw visible progression guide polyline까지의 최단거리로 읽는다. 이후 longitudinal 성분은 dominant guide 기준 coordinate를 사용한다.
...
```math
\text{center\_distance} = \min_{q \in C_g} \|p-q\|
```

여기서 `C_g`는 guide `g`의 raw visible polyline이다.
...
```math
L_{\text{linear}}(u)=\alpha u
```

현재 구현에서는 `longitudinal_peak`가 gain을 곱하기 전 함수 자체 ceiling을 직접 정한다. `linear` family에서는 `longitudinal_shape`가 ramp 기울기를 키우므로 끝단 ceiling도 함께 커진다.
...
`T(r)`는 중심에서 얼마나 벗어났는지에 대한 형태를 만든다. family와 decay에 따라 center-high profile의 날카로움과 바깥 tail이 바뀐다. 현재 구현에서 `r`은 raw visible guide polyline까지의 **미터 단위 거리 자체**다. 예전의 user-facing transverse width knob은 더 이상 active runtime contract에 포함되지 않는다.
...
detail channel 가운데 `progression_transverse_term`은 dominant guide의 actual score transverse term을 그대로 내보낸다. `score`, `s_hat`, `center_distance`, `longitudinal_component`도 모두 dominant guide 기준 값을 유지한다. 현재 `progression_center_distance`는 signed lateral이 아니라 dominant guide의 raw visible polyline까지의 unsigned 최단거리다.

현재 runtime은 `exponential`, `inverse`, `power`, `linear` 같은 transverse family를 지원한다. 현재 구현에서 `transverse_peak`는 중심 ceiling을 직접 정하고, `transverse_shape`는 중심 근처 core profile을, `transverse_falloff`는 바깥 tail suppression을 추가로 정한다. 그래서 exported `progression_transverse_term`은 `transverse_peak`에 따라 `1.0`을 넘을 수 있다.
