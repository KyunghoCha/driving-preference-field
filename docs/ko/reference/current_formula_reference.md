...
progression surface는 local map 전체에 진행 좌표와 횡방향 좌표를 동시에 부여하기 위해 필요하다. 현재 구현은 progression anchor 전체를 하나의 pool로 읽고, Gaussian weight와 soft progress gating으로 pooled coordinate field를 만든다. exported transverse는 inspection 근사가 아니라 score에 실제로 들어가는 pooled transverse와 동일하다.
...
각 progression guide는 anchor로 resample되고, 전체 anchor pool이 pooled tangent / normal 좌표 추정에 함께 쓰인다. 이 좌표가 뒤에서 pooled `s_hat`, `n_hat`를 만드는 기본 재료가 된다.
...
이 단계에서 pooled `s_hat`, `n_hat`, `t_hat`를 만든다. 이후 longitudinal / transverse 성분도 같은 pooled coordinate에서 직접 계산한다.
...
pooled transverse profile과 longitudinal gain을 더한 뒤, weak modulation을 곱해 최종 score를 만든다.

```math
\text{progression\_tilted}(p)=\text{support\_mod}\cdot\text{alignment\_mod}\cdot\left(T(r)+g\cdot L(u)\right)
```

현재 구현은 guide별 hard max envelope를 쓰지 않는다. `progression_transverse_component`는 score에 실제로 들어가는 pooled transverse term과 정확히 같다. `dominant_guides` 같은 진단 값만 pooled raw contribution 기준 상위 guide를 따로 정리한다.
