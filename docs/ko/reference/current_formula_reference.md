...
이 단계에서 guide-local `s_hat`, `n_hat`, `t_hat`를 만든다. 이후 longitudinal 성분은 dominant guide 기준 coordinate를 사용한다.
...
detail channel 가운데 `progression_transverse_component`는 near-tied guide candidate의 transverse term을 support/score 기준으로 부드럽게 섞은 inspection channel이다. 반면 `score`, `s_hat`, `n_hat`, `longitudinal_component` 등은 dominant guide 기준 값을 유지한다.
