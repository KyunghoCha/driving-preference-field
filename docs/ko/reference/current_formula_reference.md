...
이 단계에서 guide-local `s_hat`, `t_hat`를 만든다. `n_hat`는 같은 dominant guide의 raw visible progression guide polyline까지의 최단거리로 읽는다. 이후 longitudinal 성분은 dominant guide 기준 coordinate를 사용한다.
...
detail channel 가운데 `progression_transverse_component`는 near-tied guide candidate의 transverse term을 support/score 기준으로 부드럽게 섞은 inspection channel이다. 반면 `score`, `s_hat`, `n_hat`, `longitudinal_component` 등은 dominant guide 기준 값을 유지한다. 현재 `n_hat`는 signed lateral이 아니라 dominant guide의 raw visible polyline까지의 unsigned 최단거리다.
