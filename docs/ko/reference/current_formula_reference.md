...
이 단계에서 pooled `s_hat`와 `t_hat`를 만든다. 이후 횡방향 성분은 pooled local window를 다시 만들지 않고, 가장 가까운 resampled progression guide segment에 직접 투영해 읽는다.
...
guide projection으로 읽은 transverse profile과 longitudinal gain을 더한 뒤, weak modulation을 곱해 최종 score를 만든다.

```math
\text{progression\_tilted}(p)=\text{support\_mod}\cdot\text{alignment\_mod}\cdot\left(T(r)+g\cdot L(u)\right)
```

현재 구현은 guide별 hard max envelope를 쓰지 않는다. `progression_transverse_component`는 score에 실제로 들어가는 guide-projection transverse term과 정확히 같다. 이때 `n_hat`는 가장 가까운 resampled progression guide segment에 대한 signed lateral offset이다. `dominant_guides` 같은 진단 값만 pooled raw contribution 기준 상위 guide를 따로 정리한다.
