...
현재 구현의 progression surface는 guide마다 Gaussian anchor blend로 local coordinate를 만들고, guide-local score를 만든 뒤 hard max envelope로 합친다. score는 dominant guide 기준으로 유지하고, exported transverse는 near-tied guide candidate의 transverse term을 부드럽게 섞은 inspection channel로 노출한다.
