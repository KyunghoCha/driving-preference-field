...
현재 구현의 progression surface는 guide마다 Gaussian anchor blend로 local coordinate를 만들고, guide-local score를 만든 뒤 hard max envelope로 합친다. score와 exported transverse를 포함한 debug coordinate는 dominant guide 기준으로 유지한다.
