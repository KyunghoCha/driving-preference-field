...
Parameter Lab은 현재 whole-space preference field의 morphology를 반복 가능하게 비교하는 연구 도구다. 그래서 먼저 노출해야 하는 것은 field semantics를 직접 읽는 knob다. 반대로 discretization, kernel, guide-aware transverse inspection smoothing 같은 항목은 구현 품질과 성능에 영향을 주므로, 같은 급의 기본 패널이 아니라 한 단계 낮은 연구용 섹션으로 두는 편이 맞다.
