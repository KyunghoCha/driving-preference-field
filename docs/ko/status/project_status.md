...
- Gaussian anchor blended pooled coordinates + soft progress gating 현재 구현 정리
...
- 현재 구현은 Gaussian anchor blended pooled coordinates와 soft progress gating을 사용한다.
- 현재 구현 exact formula는 `support_mod * alignment_mod * (pooled_transverse + longitudinal_gain * longitudinal_component)`다.
