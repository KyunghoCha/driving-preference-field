...
2. repo root에서 `PYTHONPATH=src python -m local_reference_path_cost parameter-lab`
3. case를 고른다.
...
`Planner Lookup Progression Tilted`와 `Planner Lookup Error`는 internal compare channel이다. internal planner lookup surrogate를 exact raster와 비교해서 보여줄 뿐, canonical runtime channel은 아니다.
...
- `Main`: score semantics를 읽을 때 먼저 쓰는 항목
- `Advanced Surface`: morphology 품질, discretization, support kernel, modulation을 다룰 때 여는 항목
...
planner lookup channel을 고른 경우에는 이것을 internal acceleration 비교 surface로 읽는다. canonical inspection 기준은 계속 exact runtime channel이다.
...
- [Reference-Path Cost Model 기초](../explanation/base_field_foundation.md)
