...
from local_reference_path_cost.contracts import QueryWindow, StateSample
from local_reference_path_cost.presets import save_preset
from local_reference_path_cost.ui.async_raster_evaluator import RasterComparisonResult
from local_reference_path_cost.ui.canvas_view import RasterCanvasView, raster_to_qimage
from local_reference_path_cost.ui.locale import (
    DEFAULT_LANGUAGE,
    language_display_items,
    normalize_language,
    t,
)
from local_reference_path_cost.ui.parameter_lab.constants import CHANNEL_OPTIONS, COMPARE_LAYOUT_OPTIONS
from local_reference_path_cost.ui.parameter_lab.coordinator import ParameterLabCoordinator
from local_reference_path_cost.ui.parameter_lab.help_actions import ParameterLabHelpActions
...
from local_reference_path_cost.ui.widgets.case_panel import CasePanelWidget
from local_reference_path_cost.ui.widgets.color_scale_widget import ColorScaleWidget
from local_reference_path_cost.ui.widgets.layer_panel import LayerPanelWidget
from local_reference_path_cost.ui.widgets.preset_panel import PresetPanelWidget
from local_reference_path_cost.ui.widgets.progression_parameter_panel import ProgressionParameterPanelWidget
from local_reference_path_cost.ui.widgets.profile_panel import ProfilePanelWidget
from local_reference_path_cost.ui.widgets.summary_panel import SummaryPanelWidget
...
if TYPE_CHECKING:
    from local_reference_path_cost.profile_contracts import ComparisonProfileResult
...
class ParameterLabWindow(QMainWindow):
    def __init__(
        ...
        *,
        ...
        self._settings = QSettings("local-reference-path-cost", "ParameterLab")
        ...
        self._raster_x_samples = int(self._settings.value("visualization/x_samples", 160))
        self._raster_y_samples = int(self._settings.value("visualization/y_samples", 160))
        ...
        self._resolution_cross_label = QLabel("x")
        self._resolution_x_spin = QSpinBox()
        self._resolution_y_spin = QSpinBox()
        for spin, value in (
            (self._resolution_x_spin, self._raster_x_samples),
            (self._resolution_y_spin, self._raster_y_samples),
        ):
    ...
    def export_current_comparison(self) -> Path | None:
        if self._comparison_result is None:
            return None
        from local_reference_path_cost.profile_inspection import export_profile_bundle
        from local_reference_path_cost.rendering import render_case, render_diff_image
        ...
        baseline_artifacts = render_case(
            ...
            x_samples=self._raster_x_samples,
            y_samples=self._raster_y_samples,
        )
    ...
    def _build_toolbar(self) -> None:
        ...
        self._toolbar.addSeparator()
        self._toolbar.addWidget(self._resolution_label)
        self._toolbar.addWidget(self._resolution_x_spin)
        self._toolbar.addWidget(self._resolution_cross_label)
        self._toolbar.addWidget(self._resolution_y_spin)
        self._toolbar.addSeparator()
    ...
    def _wire_signals(self) -> None:
        ...
        self._resolution_x_spin.valueChanged.connect(self._on_resolution_changed)
        self._resolution_y_spin.valueChanged.connect(self._on_resolution_changed)
    ...
    def retranslate_ui(self) -> None:
        ...
        self._resolution_label.setText(t(self._language, "toolbar.resolution"))
    ...
    def _on_resolution_changed(self) -> None:
        self._raster_x_samples = int(self._resolution_x_spin.value())
        self._raster_y_samples = int(self._resolution_y_spin.value())
        self._settings.setValue("visualization/x_samples", self._raster_x_samples)
        self._settings.setValue("visualization/y_samples", self._raster_y_samples)
        self._reset_views_pending = True
        self._schedule_evaluation()
    ...
    def _refresh_profile_panel(self) -> None:
        if self._comparison_result is None:
            ...
            return
        try:
            from local_reference_path_cost.profile_inspection import build_comparison_profile
    ...
    def _profile_summary_payload(self) -> dict[str, object]:
        ...
        from local_reference_path_cost.profile_inspection import summarize_profile_result
