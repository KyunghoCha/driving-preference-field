...
def build_progression_lookup_on_grid(
    snapshot: SemanticInputSnapshot,
    context: QueryContext,
    *,
    x_coords: np.ndarray,
    ...
    runtime = build_field_runtime(snapshot, context, config=field_config)
    progression_tilted = runtime.query_progression_score_grid(x_coords, y_coords)
    return PreparedProgressionLookup(
        local_window=context.local_window,
        x_coords=x_coords.copy(),
        y_coords=y_coords.copy(),
        progression_tilted=progression_tilted,
        cache_key=_lookup_explicit_grid_key(snapshot, context, field_config, x_coords, y_coords),
        build_metadata={
            "grid_spacing_m": None,
            "grid_kind": "explicit_coords",
            "x_count": int(x_coords.size),
            "y_count": int(y_coords.size),
            "field_config": field_config.to_dict(),
            "window": {
...
def _lookup_explicit_grid_key(
    ...
    payload = {
        ...
        "field_config": config.to_dict(),
        "grid_kind": "explicit_coords",
        "x_coords": np.round(x_coords, decimals=9).tolist(),
        "y_coords": np.round(y_coords, decimals=9).tolist(),
    }
    encoded = json.dumps(payload, sort_keys=True, separators=(",", ":"), default=repr)
    return sha1(encoded.encode("utf-8")).hexdigest()
